...
suspend fun computeIntegralParallel(f: (Double) -> Double, a: Double, b: Double) = withContext(Dispatchers.Default) {
    val count = round(b-a).toInt()
    val d = (b - a) / count
    
    0.rangeTo(count - 1)
         .map{ async{ computeIntegral(f, a + d*it, a + d*(it + 1), 20) } }.fold(0.0){l, r -> l + r.await()}
}
...
//     println(computeIntegralParallel(f, 0.0, 4.0))
    ...
    val time1 = measureTime {
        computeIntegral(f, 0.0, 1000.0, 10000)
    }
    
    println(time1)
    
    val time2 = measureTime {
        runBlocking { computeIntegralParallel(f, 0.0, 1000.0) }
    }
